...
def transform_got10k(tracker_name, cfg_name):
    ...
    src_dir = os.path.join(result_dir, "%s/%s/got10k_test/got10k/" % (tracker_name, cfg_name))
    dest_dir = os.path.join(result_dir, "%s/%s/got10k_test/got10k_submit/" % (tracker_name, cfg_name))
